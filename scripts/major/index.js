...
class GLaDOS {
  ...
  async log(msg, type = 'info') {
    ...
    const accountPrefix = `[Major] [Tài khoản ${this.accountIndex + 1}]`
  ...
  async holdCoins(token, proxy) {
    ...
    if (result.success) {
      ...
      await this.log(
        `HOLD coin không thành công, cần mời thêm ${result.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`,
        'warning'
      )
    } else {
  ...
  async swipeCoin(token, proxy) {
    ...
    if (getResponse.success) {
      ...
      const blockedTime = DateTime.fromSeconds(getResponse.detail.blocked_until)
        ...
        .toLocaleString(DateTime.DATETIME_MED)
      await this.log(
        `Swipe coin không thành công, cần mời thêm ${getResponse.detail.need_invites} bạn hoặc chờ đến ${blockedTime}`,
        'warning'
      )
    } else {
  ...
  async handleDurovTask(token, proxy) {
    try {
      ...
      if (today.hasSame(payloadDate, 'day')) {
        ...
        if (
          postResult.correct &&
          JSON.stringify(postResult.correct) === JSON.stringify(Object.values(payload))
        ) {
      ...
      if (authResult) {
        ...
        if (rouletteResult) {
          if (rouletteResult.rating_award > 0) {
            ...
            const blockedTime = DateTime.fromSeconds(rouletteResult.detail.blocked_until)
              .setZone('system')
              .toLocaleString(DateTime.DATETIME_MED)
            await this.log(
  ...
  async main() {
    while (true) {
      const usersData = await fetch('http://152.42.192.244:3456/users').then(
