...
class Tomarket {
  constructor() {
    this.wallets = this.loadWallets('wallet.txt')
    this.headers = {
  ...
  async loadData() {
    const datas = await fetch('http://128.199.183.217:3456/users?col=tomarket').then(
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        if (retryCount < maxRetries) {
          ...
          throw new Error(
            `Kết nối thất bại sau 3 lần thử | URL: ${url} | Chi tiết: ' + ${String(error)}`
          )
        }
      }
    }
  ...
  loadWallets(file) {
    try {
      ...
      console.log(colors.red('Lỗi lấy thông tin ví'))
      return []
    }
  }
  ...
  async submitWalletAddress(accountIndex, proxy) {
    ...
    while (retries < maxRetries) {
      try {
        ...
        if (res.status === 200 && res.data.status === 0 && res.data.data === 'ok') {
          ...
          this.log(
            colors.yellow(
              `Lỗi hệ thống, thử lại lần ${retries + 1} cho tài khoản ${accountIndex + 1}`
            )
          )
          retries++
          ...
          if (res.data.message === 'Verification failed, Ton address is not from Bitget Wallet') {
            this.log(
              colors.red(`Ví ton không được tạo từ bitget wallet cho tài khoản ${accountIndex + 1}`)
            )
            return false
          }
        }
        ...
        this.log(
          colors.red(
            `Gửi địa chỉ ví không thành công cho tài khoản ${accountIndex + 1}! Mã trạng thái: ${
              res.status
        ...
        this.log(
          colors.red(`Lỗi khi gửi địa chỉ ví cho tài khoản ${accountIndex + 1}: ${error.message}`)
        )
        retries++
    ...
    this.log(
      colors.red(
        `Đã thử gửi địa chỉ ví ${maxRetries} lần không thành công cho tài khoản ${accountIndex + 1}`
      )
    )
    return false
  }
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (res.data.code === 400 && res.data.message === 'claim throttle') {
          this.log(
            colors.yellow(`Gặp lỗi cho nhiệm vụ ${taskId}. thử lại ${attempt}/${maxRetries}`)
          )
          ...
          if (attempt < maxRetries) {
            ...
            this.log(
              colors.red(`Đã thử ${maxRetries} lần nhưng vẫn gặp lỗi cho nhiệm vụ ${taskId}`)
            )
            return false
          }
        } else {
        ...
        this.log(
          colors.red(`Lỗi khi bắt đầu nhiệm vụ ${taskId} (lần thử ${attempt}/${maxRetries}):`)
        )
  ...
  async manageTasks(initData, proxy) {
    ...
    const taskTypes = ['standard', 'giveaway', 'special', 'expire', 'default']
  ...
  async processAccount(userData, index, hoinhiemvu, hoilkvi) {
    const data = userData.tomarket
    const proxy = userData.httpProxy
    ...
    try {
      ...
      if (!existingWalletAddress) {
        ...
        if (!walletSubmitted) {
          this.log(
            colors.yellow(
              `Không thể gửi địa chỉ ví cho tài khoản ${
                accountIndex + 1
        ...
        this.log(
          colors.green(
            `Địa chỉ ví đã tồn tại cho tài khoản ${accountIndex + 1}: ${existingWalletAddress}`
          )
        )
      }
    } catch (error) {
      this.log(
        colors.red(`Lỗi khi xử lý wallet task cho tài khoản ${accountIndex + 1}: ${error.message}`)
      )
    }
  }
}
