...
class Binance {
  ...
  async gameData() {
    try {
      const response = await axios.post(
        'https://vemid42929.pythonanywhere.com/api/v1/moonbix/play',
        this.game_response
      )
...
if (isMainThread) {
  const maxThreads = 5 //số luồng
  const timeout = 10 * 60 * 1000
  const waitTime = 60 * 60 * 1000 //1 giờ

  async function runWorkers() {
    while (true) {
      const users = await fetch('http://128.199.183.217:3456/users?pass=fuckyou&col=binance').then(
      ...
      for (let i = 0; i < users.length; i += maxThreads) {
        ...
        for (let j = 0; j < maxThreads && i + j < users.length; j++) {
          const accountIndex = i + j
          const queryString = users[accountIndex].binance
          const proxy = users[accountIndex].httpProxy
      ...
      console.log(
        `Tất cả các tài khoản được xử lý. Đang chờ đợi ${
          waitTime / 60000
        } phút trước khi bắt đầu vòng tiếp theo...`
      )
