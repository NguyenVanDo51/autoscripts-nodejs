  ...
  async processAccount(cookie, proxy) {
    ...
    for (let j = 0; j < points.length; j++) {
      ...
      if (result.data.success) {
        ...
        await this.log(
          `Cho ăn thành công, đã bón được ${result.data.data} sâu | Còn lại ${currentMolecule} sâu`,
          'success'
        )
      } else {
        await this.log(`Không thể bón sâu: ${result.data.msg}`, 'error')
      }
...
async function main() {
  ...
  const cookies = users.map(u => u.kucoin)
  ...
  while (true) {
    for (let i = 0; i < cookies.length; i += maxThreads) {
      ...
      console.log(
        `Đã hoàn thành xử lý ${remainingAccounts} tài khoản. Chuyển sang nhóm tài khoản tiếp theo...`
          .green
      )
    }
