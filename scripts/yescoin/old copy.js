...
class YesCoinBot {
  ...
  async getuser(token, proxy) {
    try {
      const url =
        'https://api.yescoin.gold/account/getRankingList?index=1&pageSize=1&rankType=1&userLevel=1'
      const response = await this.makeRequest('get', url, null, token, proxy)
    ...
    try {
      ...
      if (accountBuildInfo.code === 0) {
        ...
        if (swipeBotLevel >= 1 && openSwipeBot) {
          ...
          const offlineBonusInfo = await this.makeRequest(
            'get',
            offlineBonusUrl,
            null,
            token,
          ...
          if (offlineBonusInfo.code === 0 && offlineBonusInfo.data.length > 0) {
            ...
            if (claimResponse.code === 0) {
              await this.log(
                `Claim offline bonus thành công, nhận ${claimResponse.data.collectAmount} coins`,
                'success'
              )
            } else {
    ...
    try {
      ...
      if (response.code === 0) {
        ...
        if (bonusInfo.commonTaskBonusStatus === 1) {
          ...
          if (claimResponse.code === 0) {
            await this.log(
              `Claim Common Task bonus thành công | phần thưởng ${claimResponse.data.bonusAmount}`,
              'success'
            )
          }
        }
        ...
        if (bonusInfo.dailyTaskBonusStatus === 1) {
          ...
          if (claimResponse.code === 0) {
            await this.log(
              `Claim Daily Task bonus thành công | phần thưởng ${claimResponse.data.bonusAmount}`,
              'success'
            )
          }
        }
  ...
  async performDailyMissions(token, proxy) {
    try {
      ...
      const dailyMissionsResponse = await this.makeRequest(
        'get',
        dailyMissionsUrl,
        null,
        token,
  ...
  async performDailySignIn(token, proxy) {
    try {
      ...
      if (signInListResponse.code === 0) {
        ...
        if (availableSignIn) {
          ...
          if (signInResponse.code === 0) {
            ...
            await this.log(
              `Điểm danh hàng ngày thất bại: ${JSON.stringify({
        ...
        await this.log(
          `Không thể lấy danh sách điểm danh: ${JSON.stringify({
      ...
      await this.log(
        `Lỗi khi thực hiện điểm danh hàng ngày: ${error.message}
            Stack: ${error.stack}
  ...
  async performTasks() {
    try {
      ...
      if (gameInfo === null) {
        ...
        const {
          specialBoxLeftRecoveryCount,
          coinPoolLeftRecoveryCount,
          singleCoinValue,
          singleCoinLevel,
          coinPoolRecoverySpeed,
          swipeBotLevel,
        } = gameInfo.data
        await this.log(
          `Booster: Chest ${specialBoxLeftRecoveryCount} | Recovery ${coinPoolLeftRecoveryCount}`,
          'info'
        )
        await this.log(
          `Multivalue: ${singleCoinValue} | Coin Limit: ${singleCoinLevel} | Fill Rate: ${coinPoolRecoverySpeed} | Swipe Bot: ${swipeBotLevel}`,
          'info'
        )
      }
      ...
      if (this.config.upgradeFillEnable && gameInfo) {
        await this.performTaskWithTimeout(
          () =>
            this.upgradeLevel(
              this.token,
              gameInfo.data.coinPoolRecoverySpeed,
              this.config.maxLevel,
              '2',
              this.proxy
            ),
          ...
          if (updatedCollectInfo) {
            ...
            if (coinPoolLeftCount > 0) {
              ...
              if (collectResult && collectResult.code === 0) {
                const collectedAmount = collectResult.data.collectAmount
                await this.log(
                  `Tap thành công sau recovery, nhận được ${collectedAmount} coins`,
                  'success'
                )
              } else {
