...
class YesCoinBot {
    ...
    try {
      ...
      if (response.code === 0) {
        ...
        if (bonusInfo.commonTaskBonusStatus === 1) {
          ...
          if (claimResponse.code === 0) {
            await this.log(
              `Claim Task bonus thành công | phần thưởng ${claimResponse.data.bonusAmount}`,
              'success'
            )
            return true
          } else {
  ...
  async performTasks() {
    try {
      ...
      await this.log(`[YesCoin] Tài khoản: ${nickname}`, 'info')
...
if (isMainThread) {
  ...
  const numThreads = Math.min(config.maxThreads || 10)
  let activeWorkers = 0
